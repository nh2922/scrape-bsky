...
def login_bot(bot_handle: str, bot_password: str) -> Client | None:
    ...
    try:
        ...
        # This is rare for login() itself but good to be explicit
        ...
        # Catch other protocol-level errors without swallowing unrelated bugs
...
def scrape_feed_for_bot(client: Client, bot_handle: str) -> dict[str, list]:
    ...
    while len(feed_data["text"]) < TARGET:
        ...
        # --- retry loop JUST around the network call ---
        ...
        if not cursor:
            ...
            cursor = None
            # (optional: add a max_restarts guard later)
            continue
...
def append_results_to_csv(
    ...
    # IMPORTANT: we always open in "a" (append) mode → NO overwriting
    with open(OUTPUT_CSV, mode="a", newline="", encoding="utf-8") as csvfile:
        ...
        # Write results for this bot in this run (same sample_number for all bots)
        for i, item in enumerate(combined_results):
            writer.writerow({
                ...
                "Liked?": i in liked_indices,   # True if this post was liked
            })
...
def perform_likes(
    ...
    # We index into candidate_posts by its list position, but track the original
    # combined_results index via post["index"] for CSV.
    all_candidate_ids = list(range(len(candidate_posts)))
    untried_ids = set(all_candidate_ids)

    # Global guard to avoid infinite loops in extreme failure scenarios
    ...
    while len(liked_indices) < desired_likes and global_attempts < max_global_attempts:
        ...
        # Prefer posts we've never tried before
        if untried_ids:
            local_id = random.choice(tuple(untried_ids))
            untried_ids.remove(local_id)
        else:
            # Everyone has been tried at least once; retry from any candidate
            # that hasn't already been successfully liked.
            retry_pool = [
                i for i in all_candidate_ids
                if candidate_posts[i]["index"] not in liked_indices
            ]
            if not retry_pool:
                # No more candidates left that aren't already liked
                break
            local_id = random.choice(retry_pool)

        post = candidate_posts[local_id]
        orig_idx = post["index"]  # index into combined_results

        # Skip if somehow we already liked this one
        ...
        while attempt < max_retries_per_call:
            ...
            try:
                ...
                # Auth problems are fatal for this bot; stop trying likes at all.
                return liked_indices
...
def run_for_bot(bot_handle: str, bot_password: str, sample_number: int):
    ...
    # -------------------------------
    # SENTIMENT ANALYSIS WITH PER-POST RETRIES
    # -------------------------------
...
def main():
    ...
    # Compute sample_number ONCE for this entire run

...
# Custom exceptions for clearer handling upstream
...
# Global variable to cache the pipeline so it's only loaded once
...
def get_sentiment_pipeline():
    ...
    if _sentiment_pipeline is None:
        try:
            ...
            # Covers: missing model, HF hub/network problems, bad config, etc.
...
def analyze_texts(texts: List[str]) -> List[Dict[str, Any]]:
    ...
    # Basic sanity filter: coerce everything to str and drop empty strings
    ...
    try:
        ...
        # Covers tokenization issues, shape mismatches, OOM-style runtime errors, etc.
...
def analyze_and_attach(texts: List[str]) -> List[Dict[str, Any]]:
    ...
    # Note: len(results) may be <= len(texts) if some were empty and filtered out,
    # so zip() is the safest way to align.
